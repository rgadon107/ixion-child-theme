<?php
/**
 * Title: Monthly Event Group - Home Page
 * Slug: ixion-child/monthly-event-group-home-page
 * Categories: featured, events-general-home-page
 * Description: A compact version of a single event card designed for the `Home` page layout.
 * Keywords: event, monthly, compact, home, garden club
 * Viewport Width: 400
 */
?>
<!-- wp:group { "metadata": { "name": "Monthly Event Group - Home Page" }, "className": "custom-tight-card-home-page", "style": { "spacing": { "margin": { "top": "0", "bottom": "0" }, "padding": { "top": "0", "bottom": "0" } } }, "layout": { "type": "constrained" }, "templateLock": "contentOnly" } -->
<div class="wp-block-group custom-tight-card-home-page">
    ...
    <div class="wp-block-buttons register-button">
        <!-- wp:button {"backgroundColor":"brand-green-dark","textColor":"brand-white-lime","metadata":{"name":"Register for Dinner Button"},"className":"has-button","style":{"elements":{"link":{"color":{"text":"var:preset|color|brand-white-lime"}}}},"fontSize":"small"} -->
            <div class="wp-block-button has-button has-custom-font-size has-small-font-size"><a class="wp-block-button__link has-brand-white-lime-color has-brand-green-dark-background-color has-text-color has-background has-link-color wp-element-button">Register for Dinner</a></div>
        <!-- /wp:button -->
    </div>
    <!-- /wp:buttons -->
